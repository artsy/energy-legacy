...
describe(@"model behavior", ^{
    it(@"de-encodes paths for file name", ^{
        Document *document = [Document stubbedModelFromJSON:@{ @"id":@"", @"filename":@"name%2thing%2actual_name" }];
        expect(document.presentableFileName).to.equal(@"actual name");
    });

    it(@"prioritises title for presentable name", ^{
        Document *document = [Document stubbedModelFromJSON:@{ @"id":@"", @"filename":@"name", @"title": @"title" }];
        expect(document.presentableFileName).to.equal(@"title");
    });

    it(@"has a filename you'd expect", ^{
        Document *document = [Document stubbedModelFromJSON:@{ @"id":@"", @"filename":@"name%2thing%2actual_name.jpg" }];
