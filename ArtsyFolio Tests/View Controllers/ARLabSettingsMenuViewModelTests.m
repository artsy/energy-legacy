...
@interface ARLabSettingsMenuViewModel (Testing)
...
SpecBegin(ARLabSettingsMenuViewModel);

__block ARLabSettingsMenuViewModel *subject;
__block ForgeriesUserDefaults *defaults;

before(^{
    defaults = [[ForgeriesUserDefaults alloc] init];
    subject = [[ARLabSettingsMenuViewModel alloc] initWithDefaults:(id)defaults context:[CoreDataManager stubbedManagedObjectContext] appDelegate:[[ARAppDelegate alloc] init]];
