...
- (void)updateWithDictionary:(NSDictionary *)aDictionary
{
    ...
    self.slug = [aDictionary onlyStringForKey:ARFeedIDKey];
...
+ (NSArray *)autoGeneratedAlbumsInContext:(NSManagedObjectContext *)context
{
    ...
    NSArray *albums = [context executeFetchRequest:allAlbums error:nil];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"editable = NO"];
    return [albums filteredArrayUsingPredicate:predicate];
}
...
{
    NSSet *oldArtworks = self.artworks ?: [NSSet set];
    self.artworks = [NSSet setWithArray:newArtworks];
    [self updateArtists];

    AlbumEdit *edit = [AlbumEdit objectInContext:self.managedObjectContext];
    edit.album = self;
    edit.albumWasCreated = @(oldArtworks.count == 0);
    edit.createdAt = [NSDate date];

    NSMutableSet *removed = [NSMutableSet setWithSet:oldArtworks];
    [removed minusSet:[NSSet setWithArray:newArtworks]];
    edit.removedArtworks = removed;

    NSMutableSet *added = [NSMutableSet setWithSet:self.artworks];
    [added minusSet:oldArtworks];
    edit.addedArtworks = added;

    [self saveManagedObjectContextLoggingErrors];
    return edit;
}
...
- (AlbumDelete *)commitAlbumDeletion
{
    ...
    AlbumDelete *remoteDelete = [AlbumDelete objectInContext:context];
    remoteDelete.albumID = self.publicSlug;
    remoteDelete.createdAt = [NSDate date];
