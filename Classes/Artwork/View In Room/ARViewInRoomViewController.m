...
- (ARViewInRoomViewController *)initWithArtwork:(Artwork *)artwork
{
    ...
    self.view.backgroundColor = [UIColor colorWithRed:0.984 green:0.980 blue:0.980 alpha:1.000];
...
- (void)viewDidLoad
{
    ...
    [self.artworkImageView setupWithImage:self.artwork.mainImage format:ARFeedImageSizeMediumKey];
    self.artworkImageView.layer.shadowOpacity = 0.3;

    CGFloat width = 98;
    UIButton *closeButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.view.bounds) - width - 15, 14, 98, 43)];
    ...
    [closeButton setBackgroundImage:[UIImage imageNamed:@"DoneButtonBackground"] forState:UIControlStateNormal];
    [closeButton setBackgroundImage:[UIImage imageNamed:@"DoneButtonBackgroundTransparent"] forState:UIControlStateHighlighted];
...
+ (CGRect)rectForImageViewWithArtwork:(Artwork *)artwork withContainerFrame:(CGRect)containerFrame
{
    ...
    CGFloat artworkWidth = artwork.width.floatValue;
    ...
    CGFloat artworkHeight = artwork.height.floatValue;
    CGFloat artworkDiameter = artwork.diameter.floatValue;
...
{
    if (self.artwork) {
        return @{ @"artwork" : self.artwork.slug,
                  @"type" : @"artwork" };
    }
